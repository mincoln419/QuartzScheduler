...
if [ -f "server.pid" ]; then
    ...
    if ps -p $PID > /dev/null 2>&1; then
        ...
        if curl -s http://localhost:28732/actuator/health > /dev/null 2>&1; then
            echo -e "${GREEN}✅ 헬스체크 통과${NC}"
            
            # 헬스체크 상세 정보
            echo -e "${BLUE}📋 헬스체크 상세:${NC}"
            curl -s http://localhost:28732/actuator/health | jq . 2>/dev/null || curl -s http://localhost:28732/actuator/health
        else
        ...
        METRICS=$(curl -s http://localhost:28732/actuator/metrics 2>/dev/null | grep -o '"job\.[^"]*"' | head -5)
        if [ ! -z "$METRICS" ]; then
